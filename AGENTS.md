# AGENTS.md — RLALab AI Research Assistant
...
| 9 | LLM provider: Codex | `Codex-sonnet-4-6` as primary. Provider abstraction layer allows switching. No Groq/Llama in production. |
...
```
RLALab-AI-Assistant/
├── AGENTS.md                        ← This file. Architecture reference for agents.
...
│       │   ├── anthropic_provider.py← Codex (Anthropic SDK) implementation
...
### Primary: Codex (Anthropic)
- **Model:** `Codex-sonnet-4-6` (default). Configurable via `LLM_MODEL` env var.
...
Set `LLM_PROVIDER=anthropic` and `LLM_MODEL=Codex-sonnet-4-6` in `.env`.
...
Context assembly (inject chunks + metadata into prompt)
    ↓
LLM generation (Codex, streaming)
    ↓
...
7. **Rate limits:** Anthropic Codex rate limits vary by tier. Monitor `x-ratelimit-requests-remaining` and `x-ratelimit-tokens-remaining` response headers. Log these to structlog for visibility.
